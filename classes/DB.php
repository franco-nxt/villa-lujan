...
/**
 * Exception helper for the DB class
 */
...
class DB
{
    ...
    function __construct($database_name, $username, $password, $host = 'localhost')
    {
        ...
        if (!$this->mysql) {
            throw new DBException('DB connection error: ' . mysqli_connect_error());
        }
    }
     ...
     * @return DB
     */
    final public static function instance($database_name = MYSQLI_DDBB, $username = MYSQLI_USER, $password = MYSQLI_PASS, $host = MYSQLI_HOST)
    {
        if (!isset(self::$instance)) {
            self::$instance = new DB($database_name, $username, $password, $host);
        }
    ...
    {
        throw new DBException('DB error: ' . $error);
    }
    ...
    {
        if (empty($where)) {
            throw new DBException('Where clause is empty for update method');
        }
    ...
    {
        mysqli_autocommit($this->mysql ,$mode);
    }
