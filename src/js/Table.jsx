			...
			<div>
				<div className="booking">
					<div className="booking__detail">
						...
						<p className="booking__detail__info">
							<strong>Check In</strong>
							<span>{this.state.date_in}</span>
						</p>
						<p className="booking__detail__info">
							<strong>Check Out</strong>
							<span>{this.state.date_out}</span>
						</p>
					</div>
					<div className="booking__occupant">
						<h2 className="booking__detail__title">
							Inquilino: {this.state.occupant_name}
						</h2>
						<p className="booking__detail__info">
							<strong>DNI: </strong>
							<span>{this.state.occupant_dni}</span>
						</p>
						<p className="booking__detail__info">
							<strong>Telefono: </strong>
							<span>{this.state.occupant_tel}</span>
						</p>
						<p className="booking__detail__info">
							<strong>Email: </strong>
							<span>{this.state.occupant_email}</span>
						</p>
					</div>
				</div>
						...
						<thead>
							<tr>
								{this.props.cols.map(col =>
									e('th', { onClick: () => this.sort(col[1]) }, col[0])
								)}
							</tr>
						</thead>
						<tbody>
							{rows.map((row, key) =>
								e(
									'tr',
									{ key, class: this.state.id == row.id ? 'active' : null },
									this.props.cols.map(col =>
										e('td', { onClick: () => this.setState(row) }, row[col[1]])
									)
								)
							)}
						</tbody>
